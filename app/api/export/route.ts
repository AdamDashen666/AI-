...
function buildMarkdownContent(body: Omit<ExportRequestBody, "format">): string {
  ...
  return [
    ...
    "",
    "## Final Result",
    integration.finalResult,
    "",
    "## Changelog",
    ...(integration.changelog.length ? integration.changelog.map((item) => `- ${item}`) : ["- N/A"]),
    "",
    "## Remaining Problems",
    ...(integration.remainingProblems.length ? integration.remainingProblems.map((item) => `- ${item}`) : ["- N/A"]),
    "",
    "## Next Steps",
    ...(integration.nextSteps.length ? integration.nextSteps.map((item) => `- ${item}`) : ["- N/A"]),
    "",
    "## Worker Outputs",
    workerSection || "- N/A",
    "",
...
function buildTextContent(body: Omit<ExportRequestBody, "format">): string {
  return buildMarkdownContent(body)
    .replace(/^#\s+/gm, "")
    .replace(/^##\s+/gm, "")
    .replace(/\*\*/g, "");
}
